...
| 服务 | 容器名 | 端口 | 说明 |
|---|---|---|---|
| client | datapolit-client | 80 | 前端（Nginx 反代 `/core` `/api/v1` `/v1` `/doc-kit`） |
| core | datapolit-core | 3002（内部） | Node/Express 后端 |
| rag | datapolit-rag | 8000 | RAG / CharterMate（Python） |
...
| redis | datapolit-redis | 6379（内部） | Redis 语义缓存 |
...
- **RAG**：DeepSeek / 百炼密钥（小写变量名，如 `deepseek_api_key`）、`REDIS_URL=redis://redis:6379`
- **Doc-Kit**：`DASHSCOPE_API_KEY`、`DEEPSEEK_API_KEY`、`MILVUS_URI`、`MILVUS_API_KEY`（大写变量名）
- **Docs-Seeker**：`MILVUS_TOKEN`、`SEMANTIC_CACHE_ENABLED`
...
- 数据持久化使用命名卷：`mysql_data`、`redis_data`、`chroma_data`、`app_logs`、
  `docsseeker-data`、`dockit-data`；Core 日志写入 `./logs`（本目录）。
